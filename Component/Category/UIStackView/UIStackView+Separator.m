...
@class UIStackViewSeparatorHelper;
...
- (void)makeSeparators {
    ...
    [strongStackView.arrangedSubviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (strongStackView.axis == UILayoutConstraintAxisHorizontal) {
            CGFloat sepatatorLength = MIN(CGRectGetHeight(previousView.frame), self.separatorLength);
            center = (CGRectGetMaxX(previousView.frame) + CGRectGetMinX(obj.frame)) / 2.0;
            separatorView.frame = CGRectMake(center, (CGRectGetHeight(strongStackView.frame) - sepatatorLength) / 2.0, self.separatorThickness, sepatatorLength);
        }
        else {
            CGFloat sepatatorLength = MIN(CGRectGetWidth(previousView.frame), self.separatorLength);
            center = (CGRectGetMaxY(previousView.frame) + CGRectGetMinY(obj.frame)) / 2.0;
            separatorView.frame = CGRectMake((CGRectGetWidth(strongStackView.frame) - sepatatorLength) / 2.0, center, sepatatorLength, self.separatorThickness);
        }
